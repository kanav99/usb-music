...
for i in $(seq 1 $MAX_RETRIES); do
    ...
    if bluetoothctl connect "$HEADSET_MAC" 2>&1 | grep -q "Connection successful"; then
        ...
        pactl load-module module-loopback source=alsa_input.platform-3f980000.usb.stereo-fallback sink=bluez_sink.07_01_69_96_E4_3B.a2dp_sink latency_msec=100
        exit 0
    fi
